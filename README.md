...
Не забудь подключить скрипт `auth.js` в файле `auth.html` в теге `<head>` для применения JavaScript на странице.
...
Пример формирования статистики для доходов:
...
2. В папке приложения Elbrus Money Keeper выполните команду: `git init`